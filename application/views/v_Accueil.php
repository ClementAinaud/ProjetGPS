
<html lang="en">
<head>
     <title>Etapes</title>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="CSS/CSS.css">
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>


</head>
<body>
   
    
                
           <body>
        <!--<img class="imagesB" src="Images/medicament.jpg"/>-->
        <h1 align="center">Projet de navigation GPS </h1>
        <!--<img src="Images/Mr._Meeseeks" alt="[GSB]"/>-->
    
        <div class="row">
            <h2 align="center"> Bienvenue dans quelle categorie desirez vous entrer</h2>
            <br>
        
            <div id="divGenerale">
            
                <div class="col-sm-3">  
                    <img src="images/choix-dun-gps.png" class="" alt="Random Name" width="400" height="255">
                    <br> <br>
                    <h2 id="titleMed" size="6pt" type="bold" value="Gestion des médicaments">Recherche d'un itinéraire</h2> 
                      
                    <br> <br> <br>
                  <a href="index.php/Ctrl_Accueil/rechercheItineraire"><input type="button" class="btn btn-info" name="recherche " value="recherche"/></a>  
                </div>
        
          
        
            <div class="col-sm-3">            
                <img src="images/voiture.png" class="" alt="Random Name" width="400" height="255" size="700">
                <br> <br>
                <h2 id="titleComp" size="6pt" type="bold" value="Gestion des composants">Creation d'un nouveau vehicule</h2> 
               
            <br> <br> <br>
          <a href="index.php/Ctrl_Accueil/ajoutvehicule"><input type="button" class="btn btn-info" name="recherche " value="parametrages"/></a>  
            </div>
                
          <div class="col-sm-3" >
                <img src="images/parametre.png" class="" alt="Random Name" width="255" height="255">
                <br> <br> 
                <h2 id="titleVis" size="6pt" type="bold" value="Gestion des médicaments">Modification d'autres parametres</h2> 
                    
                <br> <br>
                <a href="index.php/Ctrl_Accueil/parametres"><input type="button" class="btn btn-info" name="recherche " value="parametrages"/></a> 
                
            </div>        
                
                
                
            <div class="col-sm-3">
                <img src="images/historique.jpg" class="" alt="Random Name" width="400" height="255">
                <h2 id="titlePrac" size="6pt" type="bold" value="Gestion des practiciens">Consulter l'historique de navigation</h2> 
                
                <br> <br> <br> 

           <a href="index.php/Ctrl_Accueil/historique"><input type="button" class="btn btn-info" name="recherche " value="historique"/></a> 
            </div>
        </div>
    </div><!--div row -->

<!--           
            </div>
    
    <div>
             
    </div>
            <a href="index.php/Ctrl_Accueil/historique"><input type="button" class="btn btn-info" name="recherche " value="historique"/></a>  -->
          
</body>
<!--<a href="index.php/Ctrl_Accueil/ajoutvehicule"><input type="button" class="btn btn-info" name="recherche " value="parametrages"/></a> -->
</html>
